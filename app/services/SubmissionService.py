...
class SubmissionService:
    ...
    async def submit_requirement(self,team_id:str,requirement_id:int,file:UploadFile):
        ...
        try:
            await self.srepo.add_submission(team_id,requirement_id,file.filename,doc_path,version)
        except Exception:
    ...
    async def add_remarks(self,submission_id:int,reviewer_id:int,remarks:str):
        ...
        await self.srepo.update_status(submission_id,"CHANGES_REQUIRED")
    async def add_approved(self,submission_id:int):
        await self.srepo.update_status(submission_id,"APPROVED")
